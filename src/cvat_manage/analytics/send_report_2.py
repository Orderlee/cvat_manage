...
def send_email_via_graph(targets, subject, body):
    ...
    EMAIL_RECEIVER = "amy@nobisoft.com.vn"
    access_token = get_access_token()
    today_str = datetime.today().strftime("%Y-%m-%d")
    base_path = Path(VIS_DIR)

    unique_image_image_files = set()
    for org, proj in set(targets):
        prefix = f"{org}_{proj}"
        unique_image_image_files.update([
    ...
    attachments = []
    for file_path in unique_image_image_files:
        if file_path.exists():
    ...
    message = {
        "message": {
            ...
            "toRecipients": [{"emailAddress": {"address": EMAIL_RECEIVER}}],
...
if __name__ == "__main__":
    ...
    for csv_file in recent_files:
        ...
        projects = df[df["organization"] == organization]["project"].unique()
        for project in projects:
            targets.append((organization, project))
    ...
    send_email_via_graph(targets, f"Daily CVAT Report by Annotator ({organization})", summary_text)
